...
| 37 | Build `/listings` folder for zip submission (required now that the theme ships 2 presets) | **Done 2026-09-04**, corrected twice against Shopify's real submission validator (not just docs inference): both preset folders need a real `templates/index.json` (not empty), and `/listings` can carry *only* preset folders — no README, no other files. Final shape: `listings/loam/templates/index.json` + `listings/fernway-night/templates/index.json`, nothing else. **Reverted 2026-09-10** — the resubmission audit (`CLAUDE-TASKS-theme-store-fixes.md` T4) flagged "Fernway Night" as a colour swap of "Loam," not real Theme Store differentiation, and each preset needs its own demo store to list. Collapsed to a single "Loam" preset; `/listings` no longer exists and this requirement no longer applies. See item 51 |
...
| 51 | Resubmission audit (`loam-theme-store-fixes.md`, `CLAUDE-TASKS-theme-store-fixes.md`) after a real Theme Store rejection | **In progress 2026-09-10.** Root cause: the theme shipped fabricated commercial data live on install (invented demo product cards with prices, `shopify://` refs to the developer's own demo store, B Corp/Fair Trade/review-count claims no merchant could back), not a missing-feature gap. Phase 1 (blockers) and most of Phase 2 (review polish) done — see `RELEASE-NOTES.md` 1.1.0 for the full list. Notably reverses item 37: the second "Fernway Night" preset and its `/listings` folder are gone, collapsed to a single "Loam" preset, since a colour swap doesn't count as Theme Store differentiation and a second preset needs its own demo store. Remaining: splitting `global.js` for the Lighthouse budget (needs a real browser to verify safely), the live demo store reseed, and Phase 3 (theme blocks migration / category reconsideration), which stays blocked pending Shopify's reply on whether §2 uniqueness was cited in the rejection |
