 ...
 * the theme.
 *
 * Run: node scripts/3-seed-catalog.mjs [--offline] [--dry-run]
 *
 ...
 *   --dry-run  contact the store to read it (existing products/collections,
 *              so "would create" vs. "would update" is accurate) but send no
 *              mutation — every create/update/publish call logs what it
...
import { graphql, mutate, requireCredentials, verifyShop, DRY_RUN } from './lib/shopify.mjs';
...
/* A standard definition (descriptors.care_guide) is activated, not created —
   a different mutation from a custom one. namespace/key/ownerType identify
   it directly; no template id lookup needed. */
const STANDARD_METAFIELD_DEFINITION_ENABLE = `
...
if (DRY_RUN) log.warn('Dry run: no product, collection, or metafield definition will be written.');
...
if (!offline) {
  ...
  await log.group('Metafield definitions', async () => {
    for (const definition of METAFIELD_DEFINITIONS) {
      ...
      if (definition.standard) {
        // A standard definition is activated from Shopify's own template,
        // not created — namespace/key/ownerType identify which one. type,
        // description and storefront access all come from the template
        // itself and are not settable here. Access defaults to something
        // sensible for a standard definition, but this is worth a manual
        // check the first time: Settings -> Custom data -> Products ->
        // descriptors.care_guide -> Storefront API access, before trusting
        // that the theme can actually read it.
        payload = await mutate(
          ...
          'standardMetafieldDefinitionEnable',
          // Second run: already enabled, which is success.
          { tolerate: ['TAKEN'] }
        );
      } else {
        payload = await mutate(
  ...
  await log.group('Products', async () => {
    for (const [index, product] of PRODUCTS.entries()) {
      ...
      /* Metafields. Only descriptors.care_guide — material is now a
         merchant-editable block setting and carbon_footprint was removed
         with the feature it backed (T5/T6). */
      const metafields = [
        {
          ownerId: saved.id,
...
The theme reads exactly one product metafield and degrades cleanly when it is
absent — no broken layout, the care row of the product page accordion simply
does not render. Everything else that used to be metafield-driven (the
material tag, the size guide) is now a merchant-editable setting on the block
itself, in the theme editor — nothing to set up here for those.

${METAFIELD_DEFINITIONS.map((definition) => {
  if (definition.standard) {
    return `## \`${definition.namespace}.${definition.key}\` (standard definition)

**Settings -> Custom data -> Products -> Add definition**, then look under
Shopify's own **Standard** definitions for **${definition.name}** and select
it — do not create a custom definition with this namespace and key by hand;
a standard one you create yourself will not match Shopify's own template and
can conflict with it. ${definition.description}

Standard definitions are usually already readable by the Storefront API,
but confirm under the definition's own **Storefront API access** setting
before assuming the theme can read it — a definition without that access
stores values perfectly, shows them in admin, and returns nil to Liquid.`;
  }

  return `## \`${definition.namespace}.${definition.key}\`

**Settings -> Custom data -> Products -> Add definition.** Type
\`${definition.type}\`. ${definition.description}

Under **Access -> Storefronts**, tick storefront read access — a definition
without it stores values perfectly, shows them in admin, and returns nil to
Liquid, so the theme renders nothing and the store looks broken in a way
that gives no clue where to look.`;
}).join('\n\n')}

Tick **Pin** as well. It puts the field at the top of the product editor
...
| Product | Care |
|---|---|
${PRODUCTS.map((p) => `| ${p.title} | ${p.care} |`).join('\n')}
...
log.success(
  offline
    ...
    : `${PRODUCTS.length} products, ${COLLECTIONS.length} collections.` + (DRY_RUN ? ' (dry run)' : '')
);
