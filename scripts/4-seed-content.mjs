 ...
 *
 * Run: node scripts/4-seed-content.mjs [--offline] [--dry-run]
 *
 ...
 *   --dry-run  contact the store to read it (existing pages/blog/articles/
 *              menus) but send no mutation — every create/update logs what
 *              it would have sent. The post-creation menu-nesting check is
 *              skipped, since it would otherwise re-query a menu id that was
...
if (DRY_RUN) log.warn('Dry run: no page, blog, article, or menu will be written.');
...
if (!offline) {
  ...
  await log.group('Menus', async () => {
    ...
    for (const menu of MENUS) {
      ...
      /* Verify the nesting actually took (§12.5). Skipped in dry-run mode: a
         new menu's id here is synthetic (nothing was actually created for it
         to belong to), and querying a made-up gid is a real request that
         Shopify would reject outright rather than answer with null. */
      if (DRY_RUN) continue;
...
log.success(
  offline
    ...
    : `${PAGES.length} pages, ${ARTICLES.length} articles, ${MENUS.length} menus.` + (DRY_RUN ? ' (dry run)' : '')
);
